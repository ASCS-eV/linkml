...
id: https://example.org/bidirectional-test
name: bidirectional_rule_test
prefixes:
  linkml: https://w3id.org/linkml/
  ex: https://example.org/bidirectional-test/
imports:
  - linkml:types
default_prefix: ex
default_range: string
...
classes:
  TestClass:
    ...
    slots:
      ...
      - flagValue
    rules:
      - description: Bidirectional rule.
        bidirectional: true
        preconditions:
          slot_conditions:
            flagValue:
...
_OPEN_WORLD_RULE_SCHEMA_YAML = """
id: https://example.org/open-world-test
name: open_world_rule_test
prefixes:
  linkml: https://w3id.org/linkml/
  ex: https://example.org/open-world-test/
imports:
  - linkml:types
default_prefix: ex
default_range: string
...
def test_rule_bidirectional_warns(caplog):
    """A rule with bidirectional=true emits a warning but still generates the constraint."""
    import logging

    with caplog.at_level(logging.WARNING):
        g = _parse_shacl(_BIDIRECTIONAL_RULE_SCHEMA_YAML)

    shape = URIRef("https://example.org/bidirectional-test/TestClass")
    sparql_nodes = list(g.objects(shape, SH.sparql))
    assert len(sparql_nodes) == 1, "Bidirectional rule should still emit a constraint"
    assert any("bidirectional" in r.message for r in caplog.records)


def test_rule_open_world_warns(caplog):
    """A rule with open_world=true emits a warning but still generates the constraint."""
    import logging

    with caplog.at_level(logging.WARNING):
        g = _parse_shacl(_OPEN_WORLD_RULE_SCHEMA_YAML)

    shape = URIRef("https://example.org/open-world-test/TestClass")
    sparql_nodes = list(g.objects(shape, SH.sparql))
    assert len(sparql_nodes) == 1, "Open-world rule should still emit a constraint"
    assert any("open_world" in r.message for r in caplog.records)
...
def test_rule_with_elseconditions_skipped(caplog):
    """Rules with elseconditions are skipped — emitting half a rule is incorrect."""
    import logging
    ...
    sparql_nodes = list(g.objects(shape, SH.sparql))
    assert len(sparql_nodes) == 0, f"Rule with elseconditions should not emit sh:sparql, got {len(sparql_nodes)}"
    assert any("elseconditions" in r.message for r in caplog.records)
